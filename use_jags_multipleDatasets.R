...
# Just a quick correction to the format of the p values. Looks nice this way.
pvalue1 = as.numeric(ifelse(pvalue < 0.001, formatC(pvalue, format = "e", digits = 3), format(pvalue, digits = 3)))
final <- data.frame(p0 = p0, p1 = p1, post_mu2_gt_m1 = post_mu2_gt_m1, H = H, pvalue1 = pvalue1)




# Initial Plot
plot(final$H ~ final$pvalue)

# Secondary plot. Removing Entropy == 0
final2 <- final[which(final$H > 0), ]
plot(final2$H ~ final2$pvalue)
